...
var ModalBidding = function () {
    ...
    const initForm = () => {
        ...
        const submitButton = form.querySelector('[data-modal-action-type="submit"]');
    ...
    const initSelect2Templates = () => {
        const elements = form.querySelectorAll('[data-modal-bidding-type] select');
        ...
        const format = (item) => {
            ...
            var url = 'assets/media/' + item.element.getAttribute('data-bidding-modal-option-icon');
            var img = $("<img>", {
    ...
    const handleBidOptions = () => {
        const options = form.querySelectorAll('[data-modal-bidding="option"]');
    ...
    const handleCurrencySelector = () => {
        ...
        const swapCurrency = (target) => {
            console.log(target);
            const currencies = form.querySelectorAll('[data-modal-bidding-type]');
            currencies.forEach(currency => {
                currency.classList.add('d-none');

                if (currency.getAttribute('data-modal-bidding-type') === target.id) {
    ...
    const handleCancelAction = () => {
        const cancelButton = element.querySelector('[data-modal-action-type="cancel"]');
        const closeButton = element.querySelector('[data-modal-action-type="close"]');
    ...
    return {
        init: function () {
            // Elements
            element = document.querySelector('#modal_bidding');
            form = document.getElementById('modal_bidding_form');
