...
// Определение класса
var ModalTwoFactorAuthentication = function () {
    // Private variables
    var modal;
    var modalObject;
			...
			if (smsValidator) {
				smsValidator.validate().then(function (status) {
					...
					if (status == 'Valid') {
						...
						smsSubmitButton.setAttribute('data-indicator', 'on');
						...
						setTimeout(function() {
							...
							smsSubmitButton.removeAttribute('data-indicator');
        ...
        appsSubmitButton.addEventListener('click', function (e) {
			...
			if (appsValidator) {
				appsValidator.validate().then(function (status) {
					...
					if (status == 'Valid') {
						appsSubmitButton.setAttribute('data-indicator', 'on');

						// Disable button to avoid multiple click 
						appsSubmitButton.disabled = true;

						setTimeout(function() {
							appsSubmitButton.removeAttribute('data-indicator');
    ...
    return {
        init: function () {
            // Elements
            modal = document.querySelector('#modal_two_factor_authentication');
            ...
            optionsWrapper = modal.querySelector('[data-element="options"]');
            optionsSelectButton = modal.querySelector('[data-element="options-select"]');

            smsWrapper = modal.querySelector('[data-element="sms"]');
            smsForm = modal.querySelector('[data-element="sms-form"]');
            smsSubmitButton = modal.querySelector('[data-element="sms-submit"]');
            smsCancelButton = modal.querySelector('[data-element="sms-cancel"]');

            appsWrapper = modal.querySelector('[data-element="apps"]');
            appsForm = modal.querySelector('[data-element="apps-form"]');
            appsSubmitButton = modal.querySelector('[data-element="apps-submit"]');
            appsCancelButton = modal.querySelector('[data-element="apps-cancel"]');
...
Util.onDOMContentLoaded(function() {
    ModalTwoFactorAuthentication.init();
});
