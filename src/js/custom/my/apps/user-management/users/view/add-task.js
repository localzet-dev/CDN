...
// Определение класса
var UsersAddTask = function () {
    // Shared variables
    const element = document.getElementById('modal_add_task');
    const form = element.querySelector('#modal_add_task_form');
    ...
    var initAddTask = () => {
        ...
        $("#modal_add_task_datepicker").flatpickr({
            dateFormat: "Y-m-d",
        });
    ...
    var initUpdateTaskStatus = () => {
        const allTaskMenus = document.querySelectorAll('[data-menu-id="users-tasks"]');

        allTaskMenus.forEach(el => {
            const resetButton = el.querySelector('[data-users-update-task-status="reset"]');
            const submitButton = el.querySelector('[data-users-update-task-status="submit"]');
            const taskForm = el.querySelector('[data-menu-id="users-tasks-form"]');
