...
// Определение класса
var AppEcommerceSalesSaveOrder = function () {
    // Shared variables
    var table;
    var datatable;
    ...
    const initSaveOrder = () => {
        // Init flatpickr
        $('#ecommerce_edit_order_date').flatpickr({
            altInput: true,
            altFormat: "d F, Y",
            dateFormat: "Y-m-d",
        });
        ...
        const optionFormat = (item) => {
            ...
            template += '<img src="' + item.element.getAttribute('data-select2-country') + '" class="rounded-circle h-20px me-2" alt="image"/>';
            template += item.text;
        ...
        $('#ecommerce_edit_order_billing_country').select2({
        ...
        $('#ecommerce_edit_order_shipping_country').select2({
            placeholder: "Select a country",
            minimumResultsForSearch: Infinity,
            templateSelection: optionFormat,
            templateResult: optionFormat
        ...
        table = document.querySelector('#ecommerce_edit_order_product_table');
    ...
    var handleSearchDatatable = () => {
        const filterSearch = document.querySelector('[data-ecommerce-edit-order-filter="search"]');
    ...
    const handleShippingForm = () => {
        // Select elements
        const element = document.getElementById('ecommerce_edit_order_shipping_form');
    ...
    const handleProductSelect = () => {
        ...
        const target = document.getElementById('ecommerce_edit_order_selected_products');
        const totalPrice = document.getElementById('ecommerce_edit_order_total_price');
        ...
        checkboxes.forEach(checkbox => {
            checkbox.addEventListener('change', e => {
                ...
                const product = parent.querySelector('[data-ecommerce-edit-order-filter="product"]').cloneNode(true);
                ...
                // Get product id
                const productId = product.getAttribute('data-ecommerce-edit-order-id');
                ...
                if (e.target.checked) {
                    ...
                    const selectedProduct = target.querySelector('[data-ecommerce-edit-order-id="' + productId + '"]');
                    if (selectedProduct) {
        ...
        const detectEmpty = () => {
            ...
            const products = target.querySelectorAll('[data-ecommerce-edit-order-filter="product"]');
        ...
        const calculateTotal = (products) => {
            ...
            products.forEach(product => {
                // Get product price
                const price = parseFloat(product.querySelector('[data-ecommerce-edit-order-filter="price"]').innerText);
    ...
    const handleSubmit = () => {
        ...
        // Get elements
        const form = document.getElementById('ecommerce_edit_order_form');
        const submitButton = document.getElementById('ecommerce_edit_order_submit');
...
Util.onDOMContentLoaded(function () {
    AppEcommerceSalesSaveOrder.init();
});
